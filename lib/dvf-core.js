...
function accumulate(text, acc) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (type)
      m.locaux.push({
        type,
        bati: num(c[H.surface_reelle_bati]),
        pieces: num(c[H.nombre_pieces_principales]),
        disp: c[H.numero_disposition] || '',
      });
...
function finalize(m) {
  // DVF répète parfois à l'identique une même ligne de local : bien à cheval sur
  // plusieurs parcelles, ou simple doublon d'export. On ne compte chaque local
  // qu'une fois par (disposition, type, surface bâtie, nb de pièces), sinon la
  // surface bâtie — et donc le €/m² — est multipliée par le nombre de doublons.
  const seenLoc = new Set();
  const locaux = m.locaux.filter((l) => {
    const k = `${l.disp}|${l.type}|${l.bati}|${l.pieces}`;
    if (seenLoc.has(k)) return false;
    seenLoc.add(k);
    return true;
  });

  const habit = locaux.filter((l) => l.type === 'Maison' || l.type === 'Appartement');
  const batiTotal = habit.reduce((s, l) => s + (l.bati || 0), 0);
  const terrainTotal = [...m.terrainByParcelle.values()].reduce((s, v) => s + v, 0);

  let categorie = 'Autre';
  if (locaux.some((l) => l.type === 'Maison')) categorie = 'Maison';
  else if (locaux.some((l) => l.type === 'Appartement')) categorie = 'Appartement';
  else if (locaux.some((l) => /local/i.test(l.type))) categorie = 'Local';
  ...
  return {
    ...
    valeur: m.valeur,
    categorie,
    locaux: locaux.map((l) => ({ type: l.type, bati: l.bati, pieces: l.pieces })),
